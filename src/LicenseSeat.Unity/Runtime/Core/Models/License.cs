...
namespace LicenseSeat
{
    ...
    public sealed class License
    {
        ...
        internal static License FromLicenseData(LicenseData data, string? deviceId = null)
        {
            ...
            if (!string.IsNullOrEmpty(data.StartsAt))
            {
                if (!SecurityValidation.TryParseRfc3339(data.StartsAt, out var startsAt))
                {
                    throw new ApiException("License response contains an invalid starts_at timestamp.", 200, "invalid_response");
                }
            ...
            {
                if (!SecurityValidation.TryParseRfc3339(data.ExpiresAt, out var expiresAt))
                {
                    throw new ApiException("License response contains an invalid expires_at timestamp.", 200, "invalid_response");
                }
            ...
            if (data.ActiveEntitlements != null)
            {
                ...
                foreach (var ent in data.ActiveEntitlements)
                {
                    ...
                    if (!string.IsNullOrEmpty(ent.ExpiresAt))
                    {
                        if (!SecurityValidation.TryParseRfc3339(ent.ExpiresAt, out var entExpiresAt))
                        {
                            throw new ApiException("License response contains an invalid entitlement expiry timestamp.", 200, "invalid_response");
                        }
