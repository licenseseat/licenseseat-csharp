    ...
    public sealed class ActivationResult
    {
        ...
        internal static ActivationResult FromResponse(ActivationResponse response, string deviceId)
        {
            var result = new ActivationResult
            {
                ...
                DeviceId = response.Fingerprint ?? response.DeviceId ?? deviceId,
                DeviceName = response.DeviceName,
                IpAddress = response.IpAddress,
                Metadata = response.Metadata
            };

            if (!string.IsNullOrEmpty(response.ActivatedAt))
            {
                if (!SecurityValidation.TryParseRfc3339(response.ActivatedAt, out var activatedAt))
                {
                    throw new ApiException("Activation response contains an invalid activated_at timestamp.", 200, "invalid_response");
                }
